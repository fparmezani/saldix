...
export function AddContributionModal({ goalId, goalName, onClose }: AddContributionModalProps) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      await addContribution.mutateAsync({
        ...
        input: { amount: Number(amount), contributedAt },
      });
      resetAndClose();
    } catch (cause) {
  ...
  return (
    <ModalFrame onClose={onClose}>
      ...
      >
        <h2 className="text-lg font-semibold text-white">Aportar em &quot;{goalName}&quot;</h2>
